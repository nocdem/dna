...
/* 16-byte zero-padded tag: "DNA.BLOCK.v2" (12) + 4 zeros incl. impl. NUL. */
static const uint8_t TAG_BLOCK[16] = "DNA.BLOCK.v2\0\0\0";

_Static_assert(DNA_BH2_ENC_SIZE ==
               1 + 32 + 8 + 8 + 64 + 64 + 64 + 64 + 4 + 32 + 8,
               "header v2 size drift");
...
/* Serialize the 341 BlockID-bound bytes (offsets [0,341): everything except
 * the trailing timestamp). Shared by encode and both BlockID paths so the
 * wire form and the preimage can never drift. */
static int bh2_put_bound(const dna_block_header_v2_t *h, uint8_t out[341]) {
    ...
    memcpy(out + 241, h->validator_set_hash, 64);
    put_be32(h->tx_count, out + 305);
    memcpy(out + 309, h->proposer_id, 32);
    return 0;
}
...
int dna_bh2_decode(const uint8_t *src, size_t src_len,
    ...
    memcpy(out->validator_set_hash, src + 241, 64);
    out->tx_count = get_be32(src + 305);
    memcpy(out->proposer_id, src + 309, 32);
...
int dna_bh2_genesis_block_id(const dna_block_header_v2_t *h,
    ...
    if (manifest_len == 0 || manifest_len > (size_t)DNA_BH2_MANIFEST_MAX)
        return -1;
    /* Explicit genesis semantics: height 0 AND an all-zero chain_id field
     * (the id does not exist yet — zero in the preimage, derived after). */
