...
C implementation. The seal preimage is:
    ...
    || w_read || w_write
    || max_block_env_bytes                    (u64 BE each — the policy
                                               v2 field, capacity season)
    || w_op[0..255]                           (u64 BE each)
    || op_present[0..3]                       (u64 BE each)

and the seal is SHA3-512 over those 2164 bytes.
...
def policy_hash(tag_str, version, w_base, w_callbyte, w_authbyte,
                ...
                max_block_env_bytes, w_op, present_bits):
    ...
    pre = (tag + be32(version) + be64(w_base) + be64(w_callbyte)
           ...
           + be64(w_read) + be64(w_write) + be64(max_block_env_bytes))
    for v in w_op:
        pre += be64(v)
    for m in present:
        pre += be64(m)
    assert len(pre) == 2164, len(pre)
...
def main():
    ...
    # version 2 (capacity season); w_base=7, callbyte=1, authbyte=2,
    # effect=100, effectbyte=3, read=5, write=11;
    # max_block_env_bytes=123456789 (the appended v2 field);
    # w_op[0]=50, w_op[1]=60, w_op[255]=2**63 (a deliberate >2^32 weight
    # — the u64-truncation tripwire); presence bits exactly {0, 1, 255}.
    w_op = [0] * 256
    w_op[0] = 50
    w_op[1] = 60
    w_op[255] = 2 ** 63
    print("seal     =", seal(2, 7, 1, 2, 100, 3, 5, 11, 123456789,
                             w_op, (0, 1, 255)))
    print("identity =", identity(2, 7, 1, 2, 100, 3, 5, 11, 123456789,
                                 w_op, (0, 1, 255)))
