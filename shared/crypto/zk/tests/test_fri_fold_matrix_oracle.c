/**
 * @file test_fri_fold_matrix_oracle.c
 * @brief Phase D.4 — byte-match gate for fri_fold_matrix_fp2 (log_arity > 1).
 *
 * Loads tools/vectors/fri_fold_matrix.json (~66MB, 1080 cases) and asserts
 * exact u64 limb equality between C `fri_fold_matrix_fp2` output and Plonky3
 * `TwoAdicFriFolding::fold_matrix` generic decomposition branch (commit
 * 82cfad73, fri/src/two_adic_pcs.rs:163-213).
 *
 * The harness is identical to test_fri_fold_matrix_loga1_oracle.c (Phase D.3);
 * the JSON schema is the same — only the case set differs (log_arity ∈ {2,3,4,5}).
 *
 * Exit codes:
 *   0  all cases passed
...
int main(int argc, char **argv) {
    const char *path = (argc > 1) ? argv[1] : "tools/vectors/fri_fold_matrix.json";
    ...
    printf("\n%-40s  %5d cases\n", "fri_fold_matrix total",  total);
    printf("%-40s  %5d\n",         "fri_fold_matrix passed", passed);
    printf("%-40s  %5d\n",         "fri_fold_matrix failed", failed);
    printf("\nPHASE D.4 ORACLE GATE: %s\n",
           failed == 0 ? "GREEN" : "RED");
