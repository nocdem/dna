/**
 * @file test_stark_priming.c
 * @brief P4 replay test — STARK/PCS transcript priming (Plonky3 82cfad73).
 *
 * Parses tools/vectors/stark_priming.json (produced by a REAL p3_uni_stark::prove
 * over a vendored FibonacciAir — oracle choice A), runs dnac_stark_prime_transcript
 * on the vector's commitments + opened values, and byte-matches:
 *   - sampled alpha / zeta / zeta_next       (verifier.rs:379 / :391 / :398)
 *   - primed transcript input_buf + output_buf_remaining at verify_fri entry
 *     (two_adic_pcs.rs:687-693 — proves observe order + serialization)
 *   - input_buf_len == 160, output_buf_remaining_len == 0
 *   - non-ZK / preprocessed_width==0 / trace_next present confirmations.
 *
 * Compiled with -DDNAC_TRANSCRIPT_TESTING=1 for the input_buf/output_buf
 * snapshot accessors. Per docs/plans/2026-05-30-pcs-transcript-priming-design.md.
 *
...
int main(int argc, char **argv) {
    ...
    in.is_zk = 0;                                   /* DNAC v3.0 non-ZK (config) */
    ...
    /* 8-9. exact lengths */
    if (ibl != 160 || (uint64_t)ibl != fx.input_buf_len_field) {
        fprintf(stderr, "MISMATCH field=input_buf_len: got %zu, expected 160 (vector field %llu)\n",
    ...
    if (in.is_zk != 0) { fprintf(stderr, "MISMATCH field=non_zk: is_zk=%zu (want 0)\n", in.is_zk); fails++; }
    ...
    printf("test_stark_priming: PASS\n");
    ...
    printf("  transcript primed to verify_fri entry: input_buf=%zu bytes, output_buf_remaining=%zu.\n",
           ibl, obl);
    printf("  non-ZK (is_zk=0), preprocessed_width=0, trace_next present (len=%zu), base_degree_bits=%zu.\n",
