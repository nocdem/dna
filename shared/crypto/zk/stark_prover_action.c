/**
 * @file stark_prover_action.c
 * @brief Dual-mode S1e.4 — pure-C prover for the C1 Action AIR.
 *
 * Mirrors the conf prover (stark_prover_conf.c) with the C1-AIR constants and
 * the two AIR-specific stages swapped (S1 trace = conf_action_air_generate,
 * S6 quotient = dnac_conf_action_fold_air_eval). UNSALTED (M3b leaf-salt hiding
 * deferred to S7/wallet). 0 public values. See stark_prover_action.h.
 *
...
/* Fixed C1-Action-AIR / is_zk=1 config constants. */
#define A_IS_ZK 1u
...
/* ── S6: C1 quotient values — REUSES the verifier-fold eval per row (0 publics) ── */
static dnac_prover_status_t action_quotient_values(
    ...
    for (size_t i = 0; i < q_rows; i++) {
        ...
        folder.capture = NULL;
        dnac_conf_action_fold_air_eval(&folder);
...
/* ── priming input (0 publics, num_qc=8) ── */
static void build_prime_input(dnac_action_prover_proof_t *p) {
    ...
    p->prime_in.public_values = NULL;
    p->prime_in.num_public_values = 0;
...
static void build_coms(dnac_action_prover_proof_t *p, gold_fp2_t zeta,
...
dnac_fri_status_t dnac_action_prover_proof_verify(
    ...
    if (dnac_stark_prime_transcript(vt, &p->prime_in, &out) !=
        ...
        fprintf(stderr, "action-prover self-verify: priming FAILED\n");
    ...
    if (!(gold_fp_to_u64(out.zeta.a) == gold_fp_to_u64(p->zeta.a) &&
        ...
        fprintf(stderr,
                "action-prover self-verify: zeta MISMATCH — transcript-order "
                "bug upstream\n");
        dnac_transcript_free(vt);
        return DNAC_FRI_ERR_INVALID_PROOF_SHAPE;
    }
    ...
    dnac_transcript_free(vt);
    if (fs != DNAC_FRI_OK) {
        fprintf(stderr, "action-prover self-verify: dnac_fri_verify -> %d\n",
                (int)fs);
        return fs;
    }

    /* N-chunk constraint check (0 publics). Unmerged views: trace = first A_W
     * of the merged open; chunks = first 2 of each merged 6-wide chunk. */
    {
        ...
        if (vs != DNAC_STARK_VERIFY_OK) {
            fprintf(stderr,
                    "action-prover self-verify: N-chunk constraint check -> %d\n",
                    (int)vs);
            return DNAC_FRI_ERR_INVALID_PROOF_SHAPE;
        }
    }
...
dnac_prover_status_t dnac_action_prover_prove_production(
    ...
    const size_t nd = DNAC_ACTION_PROVER_TOTAL_DRAWS(height);
    ...
    dnac_action_prover_instance_t local = *inst;
    local.draws = draws;
    local.num_draws = nd;
    const dnac_prover_status_t rc = dnac_action_prover_prove(&local, out_proof);
    /* Zeroize the codeword-blinding stream (hides the committed amounts). */
    for (volatile uint64_t *z = draws; z < draws + nd; z++) *z = 0;
    free(draws);
    return rc;
}
...
dnac_prover_status_t dnac_action_prover_prove(
    ...
    const size_t height = (size_t)1 << inst->log_height;
    if (height > STARK_PROVER_MAX_HEIGHT) return DNAC_PROVER_ERR_PARAM;
    if (inst->num_draws != DNAC_ACTION_PROVER_TOTAL_DRAWS(height)) {
        return DNAC_PROVER_ERR_PARAM;
    }
    ...
    const size_t q_size = (size_t)1 << (degree_bits + A_LOG_NUM_QC); /* == lde_h */
    ...
    dnac_action_prover_proof_t *p =
        (dnac_action_prover_proof_t *)calloc(1, sizeof(*p));
    ...
    /* ── S1: C1 construction-gate trace (no publics) ── */
    if (!conf_action_air_generate(inst->log_height, inst->value, inst->addr,
                                  inst->rcm, inst->roles, inst->pos, inst->nk,
                                  inst->ak, inst->num_notes, base_c)) {
        rc = DNAC_PROVER_ERR_RANGE;
        goto cleanup;
    }

    /* ── S2-S8 (parametric library stages; UNSALTED SE=0) ── */
    ...
    gold_fp2_t alpha, zeta, zeta_next, fri_alpha;
    {
        ...
        s = dnac_prover_trace_on_quotient_domain(lde_c, lde_h, A_RAND_W, q_size,
                                                 A_W, trace_q);
        if (s != DNAC_PROVER_OK) { rc = s; goto cleanup; }
        s = action_quotient_values(trace_q, q_size, next_step, alpha, sf, sl, st,
                                   iv, qflat);
        if (s != DNAC_PROVER_OK) { rc = s; goto cleanup; }
