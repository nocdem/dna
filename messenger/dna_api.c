...
dna_error_t dna_encrypt_message_gek(
    ...
    if (!ctx || !plaintext || !group_uuid || !gek || !sender_fingerprint ||
        !sender_sign_privkey || !ciphertext_out || !ciphertext_len_out) {
        return DNA_ERROR_INVALID_ARG;
    }
    ...
    // === SIGN THE MESSAGE ===
    // Data to sign: group_uuid(37) || gek_version(4) || nonce(12) || encrypted_payload || tag(16)
    ...
    uint32_t gek_version_net = htonl(gek_version);
    memcpy(data_to_sign + offset, &gek_version_net, 4);
    offset += 4;
    ...
    // Format:
    // [Header(12)] [Group UUID(37)] [GEK Version(4)] [Nonce(12)]
    // [Encrypted Payload] [Tag(16)] [Sig Type(1)] [Sig Size(2)] [Signature]

    dna_enc_header_t header;
    header.version = DNA_ENC_VERSION;
    header.enc_key_type = 0;  // Not used for GEK mode
    header.recipient_count = 0;  // Not used for GEK mode
    header.message_type = MSG_TYPE_GROUP_GEK;
    ...
    // GEK Version
    memcpy(final_ciphertext + offset, &gek_version_net, 4);
    offset += 4;
...
dna_error_t dna_decrypt_message_gek(
    ...
    if (!ctx || !ciphertext || !gek || !sender_dilithium_pubkey ||
        !plaintext_out || !plaintext_len_out) {
        return DNA_ERROR_INVALID_ARG;
    }
    ...
    if (header.message_type != MSG_TYPE_GROUP_GEK) {
        return DNA_ERROR_DECRYPT;
    }
    ...
    // === PARSE GEK VERSION ===
    uint32_t gek_version;
    memcpy(&gek_version, ciphertext + offset, 4);
    gek_version = ntohl(gek_version);
    offset += 4;

    if (gek_version_out) {
        *gek_version_out = gek_version;
    }
    ...
    // === VERIFY SIGNATURE ===
    // Data signed: group_uuid(37) || gek_version(4) || nonce(12) || encrypted_payload || tag(16)
    ...
    offset += 37;
    uint32_t gek_version_net = htonl(gek_version);
    memcpy(data_to_verify + offset, &gek_version_net, 4);
    offset += 4;
