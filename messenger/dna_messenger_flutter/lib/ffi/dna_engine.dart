...
// Feed v2 models removed - replaced by Channel system
...
class DnaEngine {
  ...
  Future<void> followUser(String fingerprint) async {
    final completer = Completer<void>();
    final localId = _nextLocalId++;
    ...
    final requestId = _bindings.dna_engine_follow(
      _engine,
      fpPtr.cast(),
      callback.nativeFunction.cast(),
      nullptr,
  ...
  /// Unfollow a user
  Future<void> unfollowUser(String fingerprint) async {
    final completer = Completer<void>();
    final localId = _nextLocalId++;
    ...
    final requestId = _bindings.dna_engine_unfollow(
      _engine,
      fpPtr.cast(),
      callback.nativeFunction.cast(),
      nullptr,
  ...
  bool isFollowingUser(String fingerprint) {
    final fpPtr = fingerprint.toNativeUtf8();
    try {
  ...
  // ---------------------------------------------------------------------------
  // CHANNELS - DISABLED (feature inactive, code preserved in C engine)
  // To re-enable: define DNA_CHANNELS_ENABLED, restore channel.dart model,
  // and uncomment the channel methods below.
  // ---------------------------------------------------------------------------
