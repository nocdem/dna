...
final groupedBalancesProvider = Provider<List<GroupedToken>>((ref) {
  ...
  // Sort: fiats (USDC, USDT) always on top, then alphabetical
  const fiats = {'USDC': 0, 'USDT': 1};
  result.sort((a, b) {
    final fa = fiats[a.token.toUpperCase()] ?? 99;
    final fb = fiats[b.token.toUpperCase()] ?? 99;
    if (fa != fb) return fa.compareTo(fb);
    return a.token.toUpperCase().compareTo(b.token.toUpperCase());
  });
