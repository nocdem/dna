...
int messenger_prepare_dht_from_mnemonic(const char *mnemonic) {
    ...
    // Start with derived identity (non-blocking - bootstraps in background)
    if (nodus_messenger_init(&nid) != 0) {
        QGP_LOG_ERROR(LOG_TAG_DHT, "Failed to initialize with derived identity");
...
int messenger_load_dht_identity(const char *fingerprint) {
    ...
    // Initialize Nodus with permanent identity
    QGP_LOG_INFO(LOG_TAG_DHT, ">>> DHT INIT START <<<");

    // Cleanup old connection
    nodus_messenger_close();

    // Init with permanent identity (value-copied internally)
    if (nodus_messenger_init(&nid) != 0) {
        QGP_LOG_ERROR(LOG_TAG_DHT, "Failed to initialize Nodus");
        nodus_identity_clear(&nid);
        return -1;
    }
...
/* messenger_load_dht_identity_for_engine() removed — nodus_messenger_init() replaces it */
