...
| **Group Outbox** | 7 days | `dna:group:<uuid>:out:<day>:<salt_hex>` | No | Per-sender value_id; day buckets; CORE-04 per-group salt (Phase 6 plan 04) |

**CORE-04 (Phase 6, 2026-04-14):** Group outbox keys are now salted with a
per-group 32-byte secret (`salt_hex` = 64 lowercase hex chars). The salt is
generated by the group owner at creation time via the CSPRNG
(`qgp_randombytes`) and persisted in `groups.db` (`dht_salt` column,
schema v3 from Phase 6 plan 03). Pre-existing groups lazy-provision a salt
the next time the owner rebuilds the GEK Initial Key Packet (IKP). The salt
is distributed to members inside the owner-signed IKP (`GEK2` / v2 format,
77-byte header), so non-owner members learn the salt via the same signed
channel that distributes the GEK itself. On successful `ikp_extract` the
receiver calls `group_database_set_dht_salt` to persist the salt locally.

Hard cutover: there is **no dual-read fallback** and the legacy `GEK ` IKP
magic is no longer accepted. Pre-existing unsalted group outbox entries
within their 7-day nodus TTL window may become unreachable. This is the
accepted tradeoff for closing the passive-observer cohort-correlation leak
(D-07/D-08/D-11 RESOLVED).
...
| ML-KEM-1024 | FIPS 203 | Category 5 | Key Encapsulation (used whenever the peer has published an ML-KEM key) |
| Kyber1024 | Kyber **round-3** — *not* ML-KEM/FIPS 203 ⚠ | Category 5 | Key Encapsulation — legacy fallback for peers without an ML-KEM key |
