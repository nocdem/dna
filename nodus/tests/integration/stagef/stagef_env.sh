...
#
# Everything above this line is the LEGACY / pre-Comet PBFT lane's
# ($pbft_state, current_view, `blocks`, `stagef_leader_entry`'s
# committee-snapshot derivation) and stays exactly as it was — those
# functions are read by the legacy runner's scenarios and by
# test_vset_grow_shrink.sh, which still exercises the pre-Comet lane.
# NONE of it applies on a version-3 (cometbft) chain: `pbft_state` and
# `current_view` are never written there (nodus_witness.c's
# `witness_post_open_gate` routes a version-3 chain straight into
# `witness_cmt_tick`, which never touches the legacy BFT state), and
# there is no per-node view or leader RANK to derive — cometbft's
# proposer is a weighted round-robin over accumulated priority, carried
# in the `cmt_state` KV blob, not a SQL column this harness can read.
#
# A Comet-lane scenario must NEVER call stagef_fund_on_chain /
...
#
# ⚠ THIS FILE IS SOURCED BY EVERY SCENARIO, LEGACY AND COMET ALIKE.
