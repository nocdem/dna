...
Run this harness before shipping consensus-affecting witness, BFT, Merkle,
fee, validator or chain-configuration changes, and before a chain-reset
deployment.
...
```bash
bash nodus/tests/integration/stagef/genesis_protocol.sh
bash nodus/tests/integration/stagef/genesis_protocol.sh --scenarios
```
...
1. **Phase 1** — `ctest` in `nodus/build`. The current standalone CMake
   tree registers 140 entries including benchmark-labelled tests; use
   `ctest --test-dir nodus/build -N` to inspect the configured build.
...
- `/tmp/stagef-*/user/.dna/` — fresh DNA wallet for the test
  operator. It does not use the operator's normal `~/.dna/`; isolation is
  provided through `HOME=...`.
...
- Each `nodus-server` logs to `$BASE_DIR/node$N/nodus.log`. Check
  ...
  trail.
- Committee is 7 (quorum 5). This reproduces the harness's 7-node quorum
  edges; whether it matches a particular live deployment must be verified
  operationally.
...
Rule: **Stage F supplies the repository's end-to-end libdna ↔ libnodus
parity gate.** Unit tests are necessary but do not replace that boundary test.
