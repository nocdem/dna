...
| `bench_tps_v2.sh` (in `stagef/`, not `tests/`) | Default build (`nodus-server` + `nodus-cli` from the same tree). `STAGEF_V2_PUMP_LEAVES=<N>` exported BEFORE `stagef_up_v2.sh` (suggested 400); knobs `STAGEF_BENCH_WORKERS` (M, default 7 = one per node, max 7 — a node EVICTS an older session of the same identity, `SESSION_EVICT`, so two workers on one node kill each other's handshake; measured with M=8) and `STAGEF_BENCH_DURATION_S` (D, default 600). **Run standalone** against its own fresh bring-up — it claims and churns the whole PUMP batch. rc 99 on a non-Comet cluster. | A **measurement, not a scenario**: sustained CORE SPEND TPS at today's unit budget. See "The TPS bench" below. |

#### The TPS bench (`bench_tps_v2.sh`)

**What it measures.** M parallel `nodus-cli v2-envelope spend --amount
all --count all --shard I/M` sessions of the PUMP identity keep the
mempool fed for D seconds; the script reports committed envelopes, the
...
measured max; it ends with `stagef_cmt_diff_at_floor`. Output: stdout plus
...
**What it leaves behind.** The PUMP batch claimed; every spent coin
replaced by one coin `STAGEF_PUMP_FEE_RAW` smaller; the chain many blocks
further on; `$BASE_DIR/bench/`. Nothing killed or restarted.
...
in-flight coins to ~100 across ALL workers whatever the leaf count; the
result holds at today's `NODUS_V2_GLOBAL_UNIT_BUDGET` and metering weights
only. Every wait is progress-bounded (the inline `stagef_cmt_wait_row`
