...
3. **Phase 3** — the thirteen Comet scenarios in the EXPLICIT order the script's
...
or not — `test_cmt_env_flood.sh` carries none.
...
| `STAGEF_EPOCH_BOUNDARY_BUDGET_S` | 1800 | — | Wall-clock budget `test_v2_epoch_boundary.sh` uses to decide reachability (SKIP when the boundary cannot be reached inside it). |
| `STAGEF_RULE_N_RETIRE_BUDGET_S` | 5400 | — | Wall-clock budget `test_cmt_rule_n_retire.sh` (round 2) uses to decide reachability of THREE boundaries + settle from the current tip (SKIP when they cannot be reached inside it — always true at the shipped epoch length). |
...
| Script | Purpose |
|---|---|
| `genesis_protocol_v2.sh` | The runner (above): `stagef_up_v2.sh` + thirteen explicit scenarios (round 2 adds `test_cmt_rule_n_retire.sh`) + teardown. `--scenarios` runs against an already-up cluster. Reports SKIPs separately and says in as many words that a skip is not a pass. **Leaf budget:** claim/stake consume single-use node/user leaves each; mempool_flood spends node 4/5/6/7's own leaves; `test_cmt_claim_flood.sh` (R3 W4 package C) is the only scenario that reaches for the PUMP batch, in one call; `test_v2_epoch_boundary.sh` runs after it and ordinarily finds nothing left. A second run against the SAME cluster fails on the leaf-spending scenarios — correctly. **Phase 4:** a FAILED sweep stops the processes but keeps `$BASE_DIR` for inspection; only an all-green sweep tears down. |
...
**These thirteen scenarios are the harness's entire coverage.** Read "What
...
The thirteen Comet scenarios are order-INDEPENDENT of each other's LEAVES
...
budgets (the one wall-clock number, `STAGEF_EPOCH_BOUNDARY_BUDGET_S`,
decides SKIP, never PASS).
