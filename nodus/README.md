...
```bash
cd nodus/build
ctest --output-on-failure    # 185 registered tests, 8 of them labelled bench (`ctest -LE bench` runs 177) — measured with `ctest -N` after the root-layout round (2026-09-25) deleted test_merkle_proof and test_merkle_state_root_golden
```
