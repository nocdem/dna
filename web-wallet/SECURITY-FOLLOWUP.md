...
Use the wallet's dedicated origin and bundled scripts. The supplied `deploy/Caddyfile` provides HTTPS and the HTTP `frame-ancestors 'none'` header; the HTML provides the remaining CSP. Keep the CPUNK gateway loopback-only. For that gateway build with `VITE_CPUNK_ENDPOINT=/api/cpunk/balance npm run build`; direct CPUNK reads use the ordinary build. No backend handles seeds or private keys.
