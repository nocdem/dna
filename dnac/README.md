...
**Version:** v0.18.5-ledgerv2-s9 | **TX Wire:** v2 (since v0.17.1) | **Protocol Amounts:** v1 (Transparent)
...
- **D=24 production pool state (Ledger V2 S7, 2026-08-06 — INACTIVE)** - Real, persistent, per-domain/per-pool shielded pool state, namespaced `(domain_id, pool_id)` with NO domain/pool/asset default anywhere. Shared canonical codec `shared/dnac/pool_wire.{h,c}` (pure SHA3-512, zk-include-free; tags `DNA.POOLCFG/POOLLEAF/POOLNODE/PNUL/PHIST.v1` + empty roots `DNA.E.PNUL/E.PHIST.v1`; a zero-pool domain reproduces the frozen S2 `DNA.E.POOLS.v1` byte-identically). Note roots/commitments/nullifiers are 4 canonical Goldilocks u64 lanes, 32 B BIG-ENDIAN — the exact shielded-TX-wire lane encoding. The authoritative tree is the SHIPPED consensus-side D=24 `shielded_tree` (Poseidon2 `note_merkle_compress`, capacity 2^24, FULL-before-mutation reject); the witness persists exactly the O(D) frontier + count + current root (`nodus/src/witness/nodus_witness_v2_pools.{h,c}`), appends THROUGH `shielded_tree_append`, and mutually verifies frontier↔count↔root on every load (fail-closed, never a silent rebuild; the O(count) `v2_pool_notes` list is DERIVED path-serving state, committed only via count+root). Canonical mutation order is `(global tx index, output slot)` / `(global tx index, input slot)` — strictly ascending, contiguous slots, duplicates/gaps/reorders reject. Root history: devnet **R = 720 DISTINCT finalized note roots** per pool (consensus-committed through the versioned pool config's `config_hash`; MAINNET R stays OPEN) — one entry per changed FINAL block root, quiet blocks consume nothing, the single oldest entry evicts past the limit, a retained root reappearing fails closed, and anchor acceptance is exactly membership in the retained window of `(chain_id, domain_id, pool_id)`. Nullifier set: strict inserts (no OR IGNORE), per-pool positions, and an O(1)-per-insert incremental accumulator root (`DNA.PNUL.v1` chain) — never an unbounded full-set rehash. Pool balance is a checked u64 (overflow/underflow/below-zero reject; INT64_MAX storage bound). The CORE runtime instantiates its configured native pool (`DNAC_SHIELDED_POOL_V1`, native 64-zero-byte token_id asset, D=24, R=720) through the generic OPTIONAL `state_init` activation hook — at genesis BEFORE the registry commits `genesis_state_root`, and idempotently inside the canonical activation constructor — so the committed root and the activation comparison see the same initialized state. `pools_root` is now REAL (per-domain, pool_id-ascending Merkle) and the S5/S6 supply equation's `shielded ≡ 0` placeholder is REPLACED by the real committed balances of the domain's native-asset pools (foreign-asset and foreign-domain pools are excluded — another asset's/runtime's conservation). Schema v7 (`v2_pools`/`v2_pool_notes`/`v2_pool_nullifiers`/`v2_pool_roots`; atomic 6→7 with exact column-shape verification, per-stage fault injection, version 8+ fails closed); apply-engine phase 6p + fault points F19-F25 (S1-S6 ids frozen). Everything remains INACTIVE: Type 11 admission still REJECTS unconditionally, types 12-14 stay unassigned, no live witness path calls any pool mutation, no proof result or transaction-supplied anchor can mutate or authorize anything.
- **SHIELD/UNSHIELD wire + native stateless verification (Ledger V2 S9 Gate 2, 2026-08-06 — INACTIVE)** - Transaction types `DNAC_TX_SHIELD = 12` and `DNAC_TX_UNSHIELD = 13` are now ASSIGNED and OWNED (`dnac_tx_type_t` in `dnac/include/dnac/dnac.h`, mirrored as `NODUS_W_TX_SHIELD`/`NODUS_W_TX_UNSHIELD`, `dna_tx_type_owner()` → `DNA_DOMAIN_CORE`) — **but being defined is not being live**: both REJECT unconditionally, Type 11 still terminates in an unconditional REJECT, and the V3 wire is still rejected by every live admission path (all of them gate on wire version byte 2). Type **14 stays UNASSIGNED** and type 8 stays retired; the legacy V2 deserialize type gate (`dnac/src/transaction/serialize.c`) is now the LITERAL `11` rather than the enum tail, so the frozen V2 acceptance set can never widen when the enum grows, and `nodus_witness_verify.c` gained a NAMED reject branch for 12/13 in the same position as the type-11 dispatch (verdict unchanged — both already died as a fallthrough — only the diagnosis is new). Ships the canonical **transparent-leg section v1** (`shared/dnac/tx_wire.{h,c}` §6: `tleg_version(1)=0x01 ‖ num_tin ‖ nullifier[64]×num_tin` STRICTLY ASCENDING `‖ num_tout ‖ (fp[129] ‖ amount u64 BE ≥1 ‖ seed[32])×num_tout ‖ num_signers ‖ (pubkey[2592] ‖ signature[4627])×num_signers`; `LEG_LEN = 4 + 64·num_tin + 169·num_tout + 7219·num_signers`, worst legal leg 32,608 B; a PREFIX decode reporting `consumed` — the caller hands the remainder to the shielded decoder — and POLICY-NEUTRAL by construction: no tx_type/domain/pool branch lives in the codec, the per-type count windows are native rules) with commitment `dnac_tleg_commit` = SHA3-512 over the 16-byte tag `DNA.TLEG.v1`+5×0x00 ‖ counts ‖ inputs ‖ outputs ‖ signer **PUBKEYS** — signatures deliberately EXCLUDED (a signature cannot cover itself, and the commitment must be computable before signing), `tleg_version`/`tx_type` excluded as framing (`tx_type` is bound once by `sighash_v5`'s ExecutionContext) — filling the **frozen `sighash_v5` slot at preimage offset 453**, so no offset, length, tag or field of `sighash_v5` moves and **no proof vector moves**; the empty form `DNA.E.TLEG.v1` is a distinct domain. Ships the **native stateless verifier** `dnac_v3_native_verify_stateless` (`shared/crypto/zk/native_verify_v3.{c,h}`, ZERO production callers) with a frozen check order (generic decode → type gate {11,12,13} → body split → ExecutionContext match → fee/expiry mirrors + min-fee floor → per-type count windows and boundary equalities → transparent-leg commitment → `sighash_v5` → `tx_binding` equality → type-12 signature verify → in-TX private nullifier distinctness → proof → exported deferred-state expectations) and per-type native policy — **11**: private 1..4 in / 1..4 out, `boundary_in == 0`, `boundary_out == committed_fee`; **12 SHIELD**: `priv_in == 0` (all-zero anchor and nullifier slots), `priv_out` 1..4, `num_tin ≥ 1`, `num_signers ≥ 1`, `boundary_out == 0`, `boundary_in ≥ 1`; **13 UNSHIELD**: `priv_in` 1..4, `priv_out` 0..1, `num_tin == 0`, `num_signers == 0`, exactly one transparent output, `boundary_in == 0`, `boundary_out == recipient_amount + committed_fee` (checked add) — all transparent sums checked, overflow rejects. Seven status classes are APPENDED to `dnac_shielded_verify_status_t` with values 0..17 UNMOVED (18 `ERR_TLEG_DECODE`, 19 `ERR_TLEG_ORDER` — RESERVED, declared but never assigned because the shared codec folds ordering and duplicate rejects into one `-1` — 20 `ERR_TYPE_RULE`, 21 `ERR_TLEG_ARITH`, 22 `ERR_SIG`, 23 `ERR_NF_DUP`, 24 `ERR_TIMESTAMP`). **Two seams are stated openly:** type-12 Dilithium5 verification is a CALLER-SUPPLIED function pointer (the standalone zk build cannot link `qgp_dsa87_verify` without the vendored dsa library and its circular `randombytes` dependency) and a NULL verifier on type 12 is `ERR_SIG`, never a skip; and the second seam — types 12/13 receiving no proof verification — was **CLOSED by the S9 correction pass (2026-08-06)**: the transparent-leg commitment moved into `dnac_shielded_verify_ctx_t.tleg_commit`, so all three types now run the same real aggregate verifier (tagged-empty for 11, the real `DNA.TLEG.v1` digest for 12/13), `ERR_PROOF_DEFERRED` is deleted and value 24 now carries `ERR_TIMESTAMP` (the header timestamp of a shielded type is consensus-inert and pinned to 0). The frozen 581-byte `sighash_v5`, the 45 publics and every vector are unchanged. ⚠ A production proof is 2,474,998 B against a 65,426 B V3 body cap (37.8× over, `OBL-S9-CARRIER-CAP`), so no real shielded transaction can be framed on the wire until the activation season resolves the carrier. Witness side: the compiled CORE runtime descriptor now owns `{1,2,3,11,12,13}` with rule ids 5/6 appended (`DNA_CORERULE_SHIELD_C3_REJECT`, `DNA_CORERULE_UNSHIELD_C3_REJECT`), `rt_admit_common` hard-stops 11, 12 AND 13 unconditionally **before** the pool rule so `DNAC_SHIELDED_POOL_V1` can never become an admit path, declared work units are 12=101 / 13=100 (unreachable while admission rejects), and `CORE_RULESET_HASH` was consequently RE-DERIVED (`13bc5fa9…669ada` → `e0a0bc43…6ee77429`) while the **SYSTEM digest is UNCHANGED**. The S8 V3 shielded-body codec's test debt is also closed (`dnac_txw3_shielded_encode`/`_decode`/`_check_header` had shipped with zero callers and zero tests; `nodus/tests/test_tx_wire_v3.c` now carries round-trips, a byte-exact offset KAT against hand-written literals and the full negative matrix, plus a fail-close repair to `dnac_txw3_shielded_decode` whose two early length rejects returned without zeroing `*out`).
- **Engine-owned block identity + production QC seam (Ledger V2 O14, 2026-08-17 — INGRESS STILL CLOSED)** - Closes the O13 deferral, where `v2_blocks.block_id` was written verbatim from a caller field so a block's identity was whatever the proposer said it was. `nodus_v2_block_t` no longer HAS `block_id`, `prev_block_id` or `vset_hash` inputs: the apply engine builds the canonical 413-byte header v3 in phase 13 from LOCALLY DERIVED results and computes the BlockID it persists (`dna_bh2_encode` → `dna_bh2_block_id`), while a caller may only ASSERT through `expect_*`, extending the existing root-expectation mechanism. `prev_block_id` derives from the previous committed row, `validator_set_hash` from the block-start authority snapshot, `tx_count` from the derived batch; the only two header fields a caller still supplies are the two the engine cannot derive — `proposer_id` and `timestamp` (the latter excluded from the BlockID). **No header field has two authoritative producers.** The governing snapshot is resolved PRE-BEGIN, before any mutation and before the epoch boundary runs `commit_next`, so the snapshot a boundary block itself creates can never be the snapshot that validates it; absent authority is a node FAULT (-2), never a verdict and never a fallback to a current set. Genesis is covered too: `nodus_witness_v2_genesis_ex` derives the genesis BlockID itself and a genesis with **no manifest bytes now FAILS CLOSED**, because the canonical genesis preimage takes the manifest as an explicit input and an undefined identity is not invented. Schema **v9** adds `v2_blocks.header BLOB NOT NULL` (the decomposed columns cannot reconstruct a header — `header_version`/`chain_id`/`proposer_id`/`timestamp` were never stored and three of those four are BlockID-bound), rebuilt rather than ALTERed and failing closed on a populated table, mirroring S8. NEW `nodus/src/witness/nodus_witness_v2_finalize.{c,h}` is the production acceptance seam and the FIRST production caller of `nodus_witness_v2_qc_verify`: explicit version dispatch on the encoded byte (retired v2 and unknown are two distinct fail-closed classes) → strict decode → claimed BlockID computed once → certificate verified against COMMITTED authority → *(nothing durable yet)* → execute with every committed header field as an equality assertion set by the seam → the engine's independently derived BlockID must equal the certified one → the stored bytes must re-derive it. A v3 failure NEVER falls back to legacy — that is structural (a distinct entry point, no shared retry), not a flag. **Ingress stays closed, structurally rather than by a flag:** `nodus/src/{protocol,server,client,transport}` reference no Ledger V2 identity object, so no wire message can express a v3 block, and the live tx gate still rejects non-v2 wire before signature work; opening ingress is a later season. Types 11/12/13 stay REJECT, the legacy lane and the 144-byte certificate path are byte-untouched, and ZK is unchanged.
- **Committed V2 activation authority + migration seam (Ledger V2 O15C, 2026-08-19 — COMPILE-GATED OFF)** - Legacy-lane governance tx types **15 `DNAC_TX_V2_SCHEDULE` / 16 `DNAC_TX_V2_READY`** (type 14 stays UNASSIGNED; production builds without the CMake option `NODUS_V2_ACTIVATION` reject both and cannot commit a schedule), shared activation codec `shared/dnac/activation_wire.{h,c}` (target digest D over the compiled runtime table, quorum-signed schedule/cancel, per-validator Dilithium5 readiness, terminal source binding `DNA.LEGACY.TERM.v1`), legacy state_root **v4/0x04** with the new `activation_root` leg, epoch-boundary activation machine (all-active readiness activates; postpone/auto-cancel; ACTIVE is terminal — every height above H_act is refused) and the deterministic legacy→successor migration seam (successor V2 chain derived from the committed terminal state; claim reserve == terminal migratable value; successor `utxo_set` empty pre-claims). O15C-C (same season) also fixed two live BFT liveness defects (out-of-order vote drops, view-change join suppression) — see `nodus/BUGS.md` and nodus/CLAUDE.md.
...
### Transaction types 12 / 13 — SHIELD / UNSHIELD (Ledger V2 S9, V3 wire only, NOT live)

`DNAC_TX_SHIELD = 12` and `DNAC_TX_UNSHIELD = 13` are assigned, CORE-owned
(`dna_tx_type_owner()` → `DNA_DOMAIN_CORE`) and carried **only** by the
Transaction Wire V3 carrier — they are **not** part of the v2 layout above and
never will be. Posture today:

| Type | Assigned | Carrier | Live verdict |
|---|---|---|---|
| 0..10 (except 8) | yes | v2 | normal admission |
| 8 | **retired** | — | reject |
| 11 (`SHIELDED`) | yes | v2 body / V3 body | **unconditional REJECT** |
| 12 (`SHIELD`) | **yes (S9)** | **V3 only** | **unconditional REJECT** (named branch) |
| 13 (`UNSHIELD`) | **yes (S9)** | **V3 only** | **unconditional REJECT** (named branch) |
| 14 | **no — UNASSIGNED** | — | reject (unowned) |

The v2 deserializer's type gate is the **literal `11`**, deliberately not the
enum tail, so appending 12/13 to `dnac_tx_type_t` cannot widen the frozen v2
acceptance set (0..11, with 11 rejected downstream). On the witness side
`rt_admit_common` hard-stops 11, 12 and 13 **before** the pool rule, so
`DNAC_SHIELDED_POOL_V1` can never become an admit path, and every live
admission path still gates on wire version byte 2 — a V3 transaction is
rejected as a wire before its type is ever consulted. The V3 wire, the
transparent-leg section and the native stateless verifier that back these
types are described in the Ledger V2 S1 and S9 feature entries above.
